...
// is added to *producthunt.Client without either being wrapped by an MCP
// tool or being added to phmcp.Excluded with a reason. This is the drift-
...
func TestEveryClientMethodIsWrappedOrExcluded(t *testing.T) {
	rep := mcptool.Coverage(
		reflect.TypeOf(&producthunt.Client{}),
		phmcp.Provider{}.Tools(),
		phmcp.Excluded,
	)
...
func TestToolsValidate(t *testing.T) {
	if err := mcptool.ValidateTools(phmcp.Provider{}.Tools()); err != nil {
		t.Fatal(err)
	}
}
...
func TestPlatformName(t *testing.T) {
	if got := (phmcp.Provider{}).Platform(); got != "producthunt" {
		t.Errorf("Platform() = %q, want producthunt", got)
	}
}

// TestToolsHaveProducthuntPrefix encodes the per-platform naming convention.
func TestToolsHaveProducthuntPrefix(t *testing.T) {
	const prefix = "producthunt_"
